...
int main(int argc, char* argv[]){
	...
	if (fnr_get_float(&namelist_file, "cubecalc", "edges(1)", &edges[0])) edges[0] = 1.0;
	if (fnr_get_float(&namelist_file, "cubecalc", "edges(2)", &edges[1])) edges[1] = 1.0;
	if (fnr_get_float(&namelist_file, "cubecalc", "edges(3)", &edges[2])) edges[2] = 1.0;
	printf("edges[0] %f\n", edges[0]);
